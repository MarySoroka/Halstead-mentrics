...
def findingOperators(words):
    ...
    while i < len(words):
        word = words[i]
        if len(word) == 1:
            res = operators_of_language_single.get(word,0)
            ...
            if operators_of_language_multi.get(word,0) != 0 or word_operators_of_language.get(word,0) != 0 or methods_of_language.get(word,0) != 0:
